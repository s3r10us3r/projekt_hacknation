...
def register_new():
    ...
    data = request.get_json()
    data_dict = dict(data)
    lost_item = LostItem(data_dict, session.get('id_prefix'))
    try:
        lost_item.validate()
...
@app.route('/lost_item/<id_ewidencyjny>', methods=['POST', 'PUT'])
...
@app.route('/lost_item/<id_ewidencyjny>', methods=['GET'])
...
@app.route('/form_autocomplete', methods=['POST'])
...
def mock_ai_metoda(photos):
    return {'kategoria': 'pieniadze', 'opis': 'zajebiste pieniądze'}
